...
data_H <- data |>
  ...
  mutate(
    IBH_HUM_MA = rollmean(IBH_HUM, k = 4, fill = NA, align = "right"),
    IBH_TEMP_MA = rollmean(IBH_TEMP, k = 4, fill = NA, align = "right")
  )
...
data_W <- data |>
  ...
  pivot_wider(names_from = Variable, values_from = Value)|>
  mutate(
    IBW_HUM_MA = rollmean(IBW_HUM, k = 4, fill = NA, align = "right"),
    IBW_TEMP_MA = rollmean(IBW_TEMP, k = 4, fill = NA, align = "right")
  )
...
data_T <- data |>
  ...
  pivot_wider(names_from = Variable, values_from = Value)|>
  mutate(
    IBT_TEMP_MA = rollmean(IBT_TEMP, k = 4, fill = NA, align = "right")
  )
...
data_N <- data |>
  ...
  pivot_wider(names_from = Variable, values_from = Value)|>
  mutate(
    NS_MA = rollmean(NS, k = 4, fill = NA, align = "right")
  )
...
# Taped
data_T <- data_T |>
  mutate(IBT_TEMP_02 = if_else(IBT_TEMP < 30 | IBT_TEMP > 39, 1, 0))
```
...
# Worn
data_W <- data_W |>
  mutate(IBW_TEMP_03 = if_else(abs(IBW_TEMP - lag(IBW_TEMP, default = first(IBW_TEMP))) > 4, 1, 0),
         IBW_HUM_03 = if_else(abs(IBW_HUM - lag(IBW_HUM, default = first(IBW_HUM))) > 25, 1, 0))

# Taped
data_T <- data_T |>
  mutate(IBT_TEMP_03 = if_else(abs(IBT_TEMP - lag(IBT_TEMP, default = first(IBT_TEMP))) > 2.5, 1, 0))
```
...
# House
data_H <- data_H |>
  mutate(IBH_TEMP_04 = if_else(abs(IBH_TEMP - lag(IBH_TEMP, default = first(IBH_TEMP))) < 0.0001, 1, 0),
         IBH_HUM_04 = if_else(abs(IBH_HUM - lag(IBH_HUM, default = first(IBH_HUM))) < 0.01, 1, 0))

# Worn
data_W <- data_W |>
  mutate(IBW_TEMP_04 = if_else(abs(IBW_TEMP - lag(IBW_TEMP, default = first(IBW_TEMP))) < 0.0001, 1, 0),
         IBW_HUM_04 = if_else(abs(IBW_HUM - lag(IBW_HUM, default = first(IBW_HUM))) < 0.01, 1, 0))

# Taped
data_T <- data_T |>
  mutate(IBT_TEMP_04 = if_else(abs(IBT_TEMP - lag(IBT_TEMP, default = first(IBT_TEMP))) < 0.0001, 1, 0))

# Noise 
data_N <- data_N |>
  mutate(NS_04 = ifelse(abs(NS - lag(NS, default = first(NS))) < 0.0001, 1, 0))
```
...
*I suggest we should rather flag value that have not changed over a longer period of time like 4 intervals (1 hour). For this I utilized a backwards looking moving average.*

```{r sequence small jumps moving average, echo = TRUE}
# House
data_H <- data_H |>
  mutate(IBH_TEMP_05 = if_else(IBH_TEMP == IBH_TEMP_MA, 1, 0),
         IBH_HUM_05 = if_else(IBH_HUM == IBH_HUM_MA, 1, 0))

# Worn
data_W <- data_W |>
  mutate(IBW_TEMP_05 = if_else(IBW_TEMP == IBW_TEMP_MA, 1, 0),
         IBW_HUM_05 = if_else(IBW_HUM == IBW_HUM_MA, 1, 0))

# Taped
data_T <- data_T |>
  mutate(IBT_TEMP_05 = if_else(IBT_TEMP == IBT_TEMP_MA, 1, 0))

# Noise
data_N <- data_N |>
  mutate(NS_05 = if_else(NS == NS_MA, 1, 0))
```
...
```{r comparison, fig.align='center', fig.width=6, fig.height=2.5, echo=FALSE, message=FALSE, warning=FALSE }
for (uids in unique(data$uid)) {
  ...
  ggplot(aes(x = datetime.x, y = IBT_TEMP)) +
    ...
    grid.arrange(plot_W, plot_T,ncol=2)
}
```
