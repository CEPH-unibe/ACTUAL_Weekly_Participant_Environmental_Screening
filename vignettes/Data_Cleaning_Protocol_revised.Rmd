...
rm(list = ls())
source("../MACorWIN.R")
load("/Users/ts24n298/Desktop/App_Trainride.RData")
```
...
3. Physically plausible (temperature < -10 °C etc, taped temperature < 20 °C) **especially important for taped temperature filtering**
4. Variability (variable specific threshold of multiple sigma from mean over an extended period of time) **especially important for worn temperature/humidity**
...
```{r filter for device changes, echo = FALSE}
# loop through uids
...
data_W <- data_W |>
  mutate(IBW_TEMP_04_intermediate = if_else(IBW_HUM_MSD < x, 1, 0),
         ...
         IBW_TEMP_04 = rollmean(IBW_TEMP_04_intermediate, k = 4, fill = NA, align = "left"),
         IBW_HUM_04 = rollmean(IBW_HUM_04_intermediate, k = 4, fill = NA, align = "left"))
```
...
for (uids in unique(data$uid)) {
  ...
  ggplot(aes(x = datetime)) +
    ...
    geom_line(aes(y = ifelse(IBW_HUM_all > 0,NA_real_, IBW_HUM)), color = "blue") +
    geom_line(aes(y = ifelse(IBW_TEMP_all > 0,NA_real_, IBW_TEMP)), color = "brown") +
    theme_classic() +
    lims(y = c(15, 80)) +
    labs(x = "time", y = "value", title = paste0(uids, " - WORN"))
